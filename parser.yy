...
return_type:
    ...
    ;
stmt_list:
    OP_LBRACE stmt_list_nodes OP_RBRACE {$$ = Node::add<ast::StmtL>($2);}
    | OP_LBRACE OP_RBRACE {$$ = Node::add<ast::StmtL>(Node::add<ast::StmtN>());}
    ;  ;
stmt_list_nodes:
    stmt stmt_list_nodes {$$ = Node::add<ast::StmtN>($1,$2);}
    | {$$ = Node::add<ast::StmtN>();}
    ;
assign:
...
matched_if:
    KW_IF OP_LPAREN reg_stmt OP_RPAREN stmt_list KW_ELSE if_stmt { $$ =
 Node::add<ast::IfElse>($3,$5, $7); }
